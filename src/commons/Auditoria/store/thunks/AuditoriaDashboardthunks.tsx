...
export const fetch_estado_cartera = (anio?: string, mes?: string): any => {
  return async (dispatch: Dispatch<any>) => {
    try {
      let url = `${API_ENDPOINT}/dashboard/consolidado_cartera/`;
...
export const fetch_cartera_por_nit = (
  nit: string,
  anio?: number,
  mes?: number
): any => {
  return async (dispatch: Dispatch<any>) => {
    try {
      let url = `${API_ENDPOINT}/dashboard/cartera_por_nit/?nit=${nit}`;
      
      if (anio && mes) {
        url += `&anio=${anio}&mes=${mes}`;
      }

      console.log("📌 Consultando cartera por NIT:", url);
      const res = await fetch(url);
      const data = await res.json();
      
      if (res.ok) {
        control_success("Consulta exitosa de cartera por NIT");
        return data;
      } else {
        control_error(data.error || "Error al consultar cartera por NIT");
        return null;
      }
    } catch (err) {
      console.error("❌ Error al obtener cartera por NIT:", err);
      control_error("Error al consultar cartera por NIT");
      return null;
    }
  };
};